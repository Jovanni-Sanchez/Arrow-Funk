...
class Philly extends BaseStage {
	...
	override function create() {
		if (!ClientPrefs.data.lowQuality) {
			var bg:BGSprite = new BGSprite('stages/phillyTrain/sky', -100, 0, 0.1, 0.1);
			add(bg);
		}

		var city:BGSprite = new BGSprite('stages/phillyTrain/city', -10, 0, 0.3, 0.3);
		city.setGraphicSize(Std.int(city.width * 0.85));
		city.updateHitbox();
		add(city);

		phillyLightsColors = [0xFF31A2FD, 0xFF31FD8C, 0xFFFB33F5, 0xFFFD4531, 0xFFFBA633];
		phillyWindow = new BGSprite('stages/phillyTrain/window', city.x, city.y, 0.3, 0.3);
		...
		if (!ClientPrefs.data.lowQuality) {
			var streetBehind:BGSprite = new BGSprite('stages/phillyTrain/behindTrain', -40, 50);
			add(streetBehind);
		}
		...
		phillyStreet = new BGSprite('stages/phillyTrain/street', -40, 50);
		add(phillyStreet);
	}
	...
	override function eventPushed(event:objects.Note.EventNote) {
		switch (event.event) {
			case "Philly Glow":
				...
				phillyWindowEvent = new BGSprite('stages/phillyTrain/window', phillyWindow.x, phillyWindow.y, 0.3, 0.3);
				...
				Paths.image('stages/phillyTrain/particle'); // precache philly glow particle image
